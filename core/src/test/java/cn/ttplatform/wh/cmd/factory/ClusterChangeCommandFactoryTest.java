 ...
 */
@Slf4j
public class ClusterChangeCommandFactoryTest {

    ClusterChangeCommandFactory factory;

    @Before
    public void setUp() throws Exception {
        Pool<LinkedBuffer> pool = new FixedSizeLinkedBufferPool(10);
        factory = new ClusterChangeCommandFactory(pool);
    }

    @Test
    public void getFactoryType() {
        Assert.assertEquals(DistributableType.CLUSTER_CHANGE_COMMAND, factory.getFactoryType());
    }

    @Test
    public void create() {
        ClusterChangeCommand message = ClusterChangeCommand.builder()
            .newConfig(Collections.emptySet()).id(UUID.randomUUID().toString()).build();
