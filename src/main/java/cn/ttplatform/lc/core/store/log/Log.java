...
public interface Log {
    ...
    /**
     * search entry list from {@code from} to nextIndex
     * if to-from > maxLength, then return the entry list
     * that index in [from, from + maxLength) else return
     * the entry list that index in [from, nextIndex)
     *
     * @param from include from index
    ...
    /**
     * if {@code index} and {@code term} is matched,
     * then append the {@code entries}, else return false
     *
    ...
    void advanceCommitIndex(int commitIndex,int term);
}
